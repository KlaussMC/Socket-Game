class player {
  ...
  show() {
    for (let a in noStroke(),
        ...
        imageMode(CENTER),
        textAlign(CENTER),
        this.useControls ? 0 != this.health && image(pchar, 0, 0, this.health / 2, this.health / 2) : 0 != this.health && this.visible && image(p2char, 0, 0, this.health / 2, this.health / 2),
        pop(),
        textSize(15),
        noStroke(),
        fill(255),
        this.visible && text(this.name, this.pos.x, this.pos.y - 25),
        this.projectiles)
            this.projectiles[a] && this.projectiles[a].show();
        for (let a in this.rovers)
            this.rovers[a] && this.rovers[a].show();
  }
  update() {
    this.vel = createVector(0, 0)
    if (this.useControls) {
